...
// ---------- Stock table tap-and-hold delete (WhatsApp-style highlight, no checkboxes) ----------
  ...
  el.stockTable.querySelectorAll(".stock-table-row").forEach((row) => {
    ...
    row.addEventListener("click", (e) => {
      ...
      if (stockTableSelectionActive) {
        // in selection mode, tap toggles highlight
        if (selectedStockIds.has(itemId)) {
          selectedStockIds.delete(itemId);
...
function updateStockBulkDeleteBar() {
  const checked = selectedStockIds.size;
  ...
  if (checked === 1) {
    showHeaderSelectionEditBtn(Array.from(selectedStockIds)[0]);
  } else {
    hideHeaderSelectionEditBtn();
  }
}
...
function startSupplierLongPress(e, row) {
  ...
  supplierLongPressTimer = setTimeout(() => {
    ...
    if (activeHeaderSelectionContext === "supplier") {
      // Already selecting one — pressing again (same or a different row) just
      // clears the selection and goes back to normal, rather than switching.
...
if (el.supplierList) {
  el.supplierList.addEventListener("click", (event) => {
    ...
    // If a supplier is currently selected (header showing edit/delete), any tap
    // — on the selected row or a different one — clears the selection and
    // returns to normal, rather than navigating or switching selection.
    if (activeHeaderSelectionContext === "supplier") {
      clearSupplierHeaderSelection();
      hideHeaderSelection();
      return;
...
let selectedMasterKeys = new Set(); // keys: `${supplierId}::${batchId}`
...
function startMasterLongPress(e, row) {
  ...
  longPressTimer = setTimeout(() => {
    ...
    const key = `${row.dataset.supplierId}::${row.dataset.batchId || ""}`;
    if (selectedMasterKeys.has(key)) {
      selectedMasterKeys.delete(key);
      row.classList.remove("row-selected");
    } else {
      selectedMasterKeys.add(key);
      row.classList.add("row-selected");
    }
...
if (el.bifurcatedOrderContainer) {
  el.bifurcatedOrderContainer.addEventListener("click", (event) => {
    ...
    if (el.masterView && el.masterView.classList.contains("selection-active")) {
      ...
      if (targetRow) {
        const key = `${targetRow.dataset.supplierId}::${targetRow.dataset.batchId || ""}`;
        if (selectedMasterKeys.has(key)) {
          selectedMasterKeys.delete(key);
...
let selectedDeepLineIds = new Set();
...
function toggleDeepLineSelection(card) {
  const lineId = card.dataset.lineId;
  if (selectedDeepLineIds.has(lineId)) {
    selectedDeepLineIds.delete(lineId);
    card.classList.remove("row-selected");
  } else {
    selectedDeepLineIds.add(lineId);
...
if (el.deepViewLinesList) {
  el.deepViewLinesList.addEventListener("click", (event) => {
    ...
    if (el.deepView && el.deepView.classList.contains("selection-active")) {
      const card = event.target.closest(".order-card");
      if (card) toggleDeepLineSelection(card);
    }
  });
}
...
function updateMasterBulkDeleteToolbarState() {
  const count = selectedMasterKeys.size;
...
function updateBulkDeleteToolbarState() {
  const count = selectedDeepLineIds.size;
...
if (el.bulkDeleteExecuteBtn) {
  el.bulkDeleteExecuteBtn.addEventListener("click", async () => {
    const idsToDelete = Array.from(selectedDeepLineIds);
    if (!idsToDelete.length) return;

    if (await showConfirm(
      "Delete Items",
      `Remove these ${idsToDelete.length} selected item${idsToDelete.length === 1 ? "" : "s"} from your active order?`
    )) {
...
if (el.bulkDeleteCancelBtn) {
  el.bulkDeleteCancelBtn.addEventListener("click", () => {
    // Clear selection, un-highlight cards, hide toolbar, exit selection mode
    selectedDeepLineIds.clear();
    if (el.deepViewLinesList) {
...
if (el.masterBulkDeleteExecuteBtn) {
  el.masterBulkDeleteExecuteBtn.addEventListener("click", async () => {
    const pairsToDelete = new Set(selectedMasterKeys);
    if (!pairsToDelete.size) return;

    if (await showConfirm(
      "Delete Completed Orders",
      `Permanently delete ${pairsToDelete.size} selected completed order${pairsToDelete.size === 1 ? "" : "s"}? This cannot be undone.`
    )) {
...
if (el.masterBulkDeleteCancelBtn) {
  el.masterBulkDeleteCancelBtn.addEventListener("click", () => {
    // Clear selection, un-highlight rows, hide toolbar, exit selection mode
    selectedMasterKeys.clear();
    if (el.bifurcatedOrderContainer) {
